...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($userId, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $jobsIds, 'jobs' => $jobsIds, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
        }
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == UserModel::CUSTOMER_ROLE_ID) {
            $cuser = $user;
            ...
            if (isset($data['customer_phone_type']) && $data['customer_phone_type']) {
                $data['customer_phone_type'] = 'yes';
            } else {
                $data['customer_phone_type'] = 'no';
            }

            if (isset($data['customer_physical_type']) && $data['customer_physical_type']) {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                foreach ($jobs as $oneJob) {
                    if ($oneJob->id) { // one potential job is the same with current job
                        $userId = $oneUser->id;
                        ...
                        $logger->addInfo('Push send for job ' . $oneJob->id, [$translator_array, $delpay_translator_array, $msg_text, $data]);
                        $this->sendPushNotificationToSpecificUsers($translator_array, $oneJob->id, $data, $msg_text, false);       // send new booking push to suitable translators(not delay)
                        $this->sendPushNotificationToSpecificUsers($delpay_translator_array, $oneJob->id, $data, $msg_text, true); // send new booking push to suitable translators(need to delay)                
                    }
                }
            }
        }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == UserModel::SUPERADMIN_ROLE_ID) {
            $allJobs = Job::query();
